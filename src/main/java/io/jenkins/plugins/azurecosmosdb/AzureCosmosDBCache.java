...
public class AzureCosmosDBCache {
  ...
  private static final LoadingCache<CacheKey, CosmosClient> CACHE =
      Caffeine.newBuilder()
  ...
  public static CosmosClient get(String credentialsId, Item item) {
    AzureCosmosDBCredentials credentials =
        lookupCredentials(credentialsId, item, AzureCosmosDBCredentials.class);

    StandardCredentials authCreds =
        lookupCredentials(credentials.getCredentialsId(), item, StandardCredentials.class);

    return CACHE.get(
        new CacheKey(authCreds, credentials.getUrl(), credentials.getPreferredRegion()));
  }

  private static <T extends StandardCredentials> T lookupCredentials(
      String credentialsId, Item item, Class<T> type) {
  ...
  static class CacheKey {
    ...
    public boolean equals(Object o) {
      ...
      if (cosmosAuthCredentials instanceof AzureImdsCredentials) {
        ...
        AzureImdsCredentials cacheKeyImdsCreds =
            (AzureImdsCredentials) cacheKey.cosmosAuthCredentials;

        return Objects.equals(
            imdsCredentials.getSubscriptionId(), cacheKeyImdsCreds.getSubscriptionId());
      }
      ...
      if (cosmosAuthCredentials instanceof AzureCredentials) {
        ...
        AzureCredentials cacheKeyAzureCredentials =
            (AzureCredentials) cacheKey.cosmosAuthCredentials;
        ...
        return Objects.equals(
            ...
            && Objects.equals(
                azureCredentials.getClientId(), cacheKeyAzureCredentials.getClientId())
            && Objects.equals(
      ...
      if (cosmosAuthCredentials instanceof AzureCosmosDBKeyCredentials) {
        AzureCosmosDBKeyCredentials keyCredentials =
            (AzureCosmosDBKeyCredentials) cosmosAuthCredentials;
  ...
  private static CosmosClient createClient(CacheKey cacheKey) {
    return CredentialsHelper.createClient(
        cacheKey.cosmosAuthCredentials, cacheKey.preferredRegion, cacheKey.url);
  }
}
